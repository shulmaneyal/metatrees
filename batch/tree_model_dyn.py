...
class Tree(nn.Module):
    def __init__(self, weights, biases, beta, right, left, dist_func, temperature, stop=None, leaf=None):
    ...
    def predict(self, x):
        my_leaf_pred = self.leaf.predict(x)

        right_child_pred = self.right.predict(x)
        left_child_pred = self.left.predict(x)

        right_probs = self.get_right_probs(x)
        left_probs = 1 - right_probs

        next_level_pred = left_probs * left_child_pred + right_probs * right_child_pred
        my_pred = self.stop * my_leaf_pred + (1 - self.stop) * next_level_pred
        return my_pred

    def predict_hard(self, x):
        my_leaf_pred = self.leaf.predict_hard(x)

        right_child_pred = self.right.predict_hard(x)
        left_child_pred = self.left.predict_hard(x)
...
class TreeBuilder(nn.Module):
    ...
    def get_node(self, r, representations, x, weights, height):
        ...
        stop = (right_weights.sum(1) == 0) | (left_weights.sum(1) == 0)
        leaf = self.leaf_builder.get_node(representation)
        if stop.all():
            return leaf
        stop = stop.reshape(-1, 1, 1).float()
...
def get_regression_tree(x_dim, min_y, max_y, r_dim, height, dist_func, use_weights=False):
    leaf_builder = LeafBuilderRegression(r_dim, min_y, max_y)
    model = TreeModel(x_dim, r_dim, 1, height, leaf_builder, dist_func, use_weights)
    return model
